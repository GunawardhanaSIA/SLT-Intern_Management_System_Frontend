...
class StandardPDFService {
  ...
  addEnhancedHeader() {
    try {
      const colorScheme = this.colorSchemes[this.userSettings?.colorScheme || 'professional'];
      const fontSize = this.fontSizes[this.userSettings?.fontSize || 'medium'];
      const primaryColor = this.hexToRgb(colorScheme.primary);
      ...
      // Reset colors and position
      const textColor = this.hexToRgb(colorScheme.text);
      ...
      this.doc.setFontSize(fontSize.caption);
      this.doc.setFont(undefined, 'normal');
      const lightTextColor = this.hexToRgb(colorScheme.lightText);
      this.doc.setTextColor(lightTextColor.r, lightTextColor.g, lightTextColor.b);
      ...
      // Add decorative line
      const borderColor = this.hexToRgb(colorScheme.border);
  ...
  addTableOfContents() {
    ...
    this.exportOptions.forEach((option, index) => {
      ...
      const titleWidth = this.doc.getTextWidth(`${index + 1}. ${title}`);
      const pageNumWidth = this.doc.getTextWidth(pageRef.toString());
      const dotsWidth = this.pageWidth - this.margin - titleWidth - pageNumWidth - 30;
      const dotCount = Math.floor(dotsWidth / 3);
      
      this.doc.setTextColor(lightTextColor.r, lightTextColor.g, lightTextColor.b);
      this.doc.text('.'.repeat(dotCount), this.margin + titleWidth + 10, this.currentY);
      
      this.doc.setTextColor(textColor.r, textColor.g, textColor.b);
      this.doc.text(pageRef.toString(), this.pageWidth - this.margin - 15, this.currentY);
  ...
  addSectionTitle(title, subtitle = null) {
    ...
    // Section background
    const bgColor = this.hexToRgb(colorScheme.background);
  ...
  drawMetricCard(x, y, width, height, metric, colorScheme, fontSize) {
    ...
    // Accent bar
    const accentColor = this.hexToRgb(metric.color);
  ...
  // Method to draw a pie chart using arc approximation
  drawPieChart(x, y, radius, data, title) {
    const total = data.reduce((sum, item) => sum + item.value, 0);
    if (total === 0) return;
    ...
    data.forEach((item, index) => {
      ...
      // Draw slice using multiple triangles for smooth appearance
      const segments = Math.max(1, Math.ceil(sliceAngle / (Math.PI / 12))); // 12 segments per full circle
      ...
      for (let i = 0; i < segments; i++) {
        ...
        this.doc.triangle(x, y, x1, y1, x2, y2, 'FD');
      }
  ...
  drawBarChart(x, y, width, height, data, title) {
    ...
    const maxValue = Math.max(...data.map(d => d.value));
    const barWidth = (width - 20) / data.length * 0.8;
    const barSpacing = (width - 20) / data.length * 0.2;
    
    // Draw bars
    data.forEach((item, index) => {
      const barHeight = maxValue > 0 ? (item.value / maxValue) * height : 0;
      const barX = x + 10 + (index * (barWidth + barSpacing));
      const barY = y + height - barHeight;
      ...
      // Category label
      const labelText = item.name.length > 8 ? item.name.substring(0, 8) + '...' : item.name;
      this.doc.text(labelText, barX + barWidth / 2, y + height + 8, { align: 'center' });
    });
    ...
    this.doc.setDrawColor(borderColor.r, borderColor.g, borderColor.b);
    this.doc.setLineWidth(0.5);
    this.doc.line(x + 10, y + height, x + width - 10, y + height); // X-axis
    this.doc.line(x + 10, y, x + 10, y + height); // Y-axis
  }
