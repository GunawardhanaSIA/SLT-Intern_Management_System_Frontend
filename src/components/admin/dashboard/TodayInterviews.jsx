...
const TodayInterviews = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        const filteredApplicants = response.data.filter(
          (applicant) =>
            applicant.interviewDate === new Date().toISOString().split("T")[0]
        );
    ...
    axios
      .get(`${API_BASE_URL}/admin/supervisors`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleSaveIntern = (applicant, selectedSupervisor) => {
    ...
    try {
      const postResponse = fetch(
        `${API_BASE_URL}/admin/createIntern?userId=${applicant.user.id}&applicantId=${applicant.applicantId}&supervisorId=${selectedSupervisor}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      setApplicants((prevApplicants) =>
        prevApplicants.filter((a) => a.applicantId !== applicant.applicantId)
      );
  ...
  return (
    <div className="">
      <div className="flex justify-between items-center">
        ...
        <h1 className="font-bold text-lg text-zinc-600">
                  ...
                  <div className="relative flex items-center gap-2">
                    ...
                    <Tooltip color="success" content="Accept Applicant">
                      <span className="text-lg text-success cursor-pointer active:opacity-50">
                        <FaUserCheck
                          onClick={() => handleSupervisorModal(applicant)}
                        />
                      </span>
                    </Tooltip>
            ...
            {selectedApplicant ? (
              <div>
                ...
                <div className="grid grid-cols-2 gap-y-5 gap-x-10 m-auto">
                  ...
                  <Input
                    isDisabled
                    label="12. Familiar with Programming Languages?"
                    labelPlacement="outside"
      ...
      >
        <ModalContent>
          <ModalHeader>
            Select Supervisor for {selectedApplicant?.name}
          </ModalHeader>
          <ModalBody>
            <Select
              className="my-8"
          ...
          <ModalFooter>
            <Button
              variant="bordered"
              color="danger"
              onPress={() => setIsSupervisorModalOpen(false)}
            >
              Cancel
            </Button>
            <Button
              ...
              onPress={() =>
                handleSaveIntern(selectedApplicant, selectedSupervisor)
              }
            >
              Save Intern
            </Button>
