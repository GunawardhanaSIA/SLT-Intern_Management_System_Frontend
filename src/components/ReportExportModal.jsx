...
function ReportExportModal({ reportData, onExport }) {
  ...
  const exportOptionDescriptions = {
    overview:
      "Total counts, percentages, and key metrics with visual summaries",
    statusDistributions:
      "Intern and supervisor status breakdowns with pie charts",
    monthlyTrends: "Registration trends over time with interactive line charts",
    specializations:
      "Distribution by specialization and department with bar charts",
    topPerformers:
      "Highest performing interns with detailed performance metrics",
  ...
  const formatOptions = [
    {
      key: "A4",
      label: "A4 (Standard)",
      description: "Most common format (210×297mm)",
    },
    {
      key: "A3",
      label: "A3 (Large)",
      description: "Larger format for detailed charts (297×420mm)",
    },
    {
      key: "Letter",
      label: "Letter (US)",
      description: "US standard format (8.5×11 inches)",
    },
    {
      key: "Legal",
      label: "Legal (US)",
      description: "US legal format (8.5×14 inches)",
    },
  ];
  ...
  const colorSchemes = [
    {
      ...
      description: "Corporate blue and gray theme",
    },
    {
      key: "modern",
      label: "Modern Green",
  ...
  const fontSizes = [
    {
      key: "small",
      label: "Small (10-12pt)",
      description: "Compact layout, more content per page",
    },
    {
      key: "medium",
      label: "Medium (12-14pt)",
    ...
    try {
      if (format === "pdf") {
        ...
        try {
          ...
          console.warn(
            "Enhanced PDF service failed, trying fallback:",
            autoTableError
          );
        ...
        if (downloadSuccess) {
          ...
          setExportStatus(
            "Download ready - click manual download button below"
          );
        }
      } else if (format === "csv") {
      ...
      if (
        ...
        setExportStatus(
          "Report too large. Try reducing the number of sections."
        );
      } else {
  ...
  const generateCSVExport = async () => {
    try {
      ...
      if (
        exportOptions.includes("internDetails") ||
        exportOptions.includes("departments")
      ) {
        try {
          const token = getToken();
          const internsResponse = await axios.get(
            `${API_BASE_URL}/admin/interns`,
            {
              headers: {
      ...
      if (exportOptions.includes("supervisorDetails")) {
        try {
          ...
          const supervisorsResponse = await axios.get(
            `${API_BASE_URL}/admin/supervisors`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
      ...
      if (
        exportOptions.includes("internDetails") &&
        detailedInterns.length > 0
      ) {
        csv += "DETAILED INTERN INFORMATION\n";
        csv +=
          "Intern ID,Name,Email,Mobile,NIC,Address,Educational Institute,Degree,Academic Year,Specialization,Programming Languages,Supervisor,Start Date,Status\n";
        ...
        detailedInterns.forEach((intern) => {
          ...
          const address = (intern.address || "")
            .replace(/,/g, ";")
            .replace(/"/g, '""');
          const institute = (intern.educationalInstitute || "")
            .replace(/,/g, ";")
            .replace(/"/g, '""');
          ...
          const specialization = (intern.specialization || "")
            .replace(/,/g, ";")
            .replace(/"/g, '""');
          const programmingLanguages = (intern.programmingLanguages || "")
            .replace(/,/g, ";")
            .replace(/"/g, '""');
          const supervisor = (intern.supervisor?.name || "")
            .replace(/,/g, ";")
            .replace(/"/g, '""');
      ...
      if (
        exportOptions.includes("supervisorDetails") &&
        detailedSupervisors.length > 0
      ) {
        csv += "DETAILED SUPERVISOR INFORMATION\n";
        csv +=
          "Supervisor ID,Name,Email,Mobile,NIC,Address,Technology,Experience,Status\n";
        ...
        detailedSupervisors.forEach((supervisor) => {
          ...
          const address = (supervisor.address || "")
            .replace(/,/g, ";")
            .replace(/"/g, '""');
          const technology = (supervisor.technology || "")
            .replace(/,/g, ";")
            .replace(/"/g, '""');
      ...
      if (exportOptions.includes("statusDistributions")) {
        if (reportData.internStatusDistribution) {
          ...
          csv += "Status,Count\n";
          Object.entries(reportData.internStatusDistribution).forEach(
            ([key, value]) => {
              csv += `${key},${value}\n`;
            }
          );
        ...
        if (reportData.supervisorStatusDistribution) {
          ...
          csv += "Status,Count\n";
          Object.entries(reportData.supervisorStatusDistribution).forEach(
            ([key, value]) => {
              csv += `${key},${value}\n`;
            }
          );
      ...
      if (
        exportOptions.includes("specializations") &&
        reportData.specializationDistribution
      ) {
        csv += "SPECIALIZATION DISTRIBUTION\n";
        csv += "Specialization,Count\n";
        Object.entries(reportData.specializationDistribution).forEach(
          ([key, value]) => {
            csv += `${key},${value}\n`;
          }
        );
      ...
      if (
        exportOptions.includes("monthlyTrends") &&
        reportData.monthlyRegistrations
      ) {
        csv += "MONTHLY REGISTRATION TRENDS\n";
        csv += "Month,Registrations\n";
        Object.entries(reportData.monthlyRegistrations).forEach(
          ([month, count]) => {
            csv += `${month},${count}\n`;
          }
        );
      ...
      if (exportOptions.includes("topPerformers") && reportData.topPerformers) {
        csv += "TOP PERFORMERS\n";
        csv +=
          "Name,Performance Score,Total Hours,Attendance Rate,Specialization\n";
        reportData.topPerformers.forEach((performer) => {
          ...
          const specialization = (performer.specialization || "N/A").replace(
            /,/g,
            ";"
          );
      ...
      if (
        ...
        Object.entries(reportData.departmentDistribution).forEach(
          ([key, value]) => {
            csv += `${key},${value}\n`;
          }
        );
      ...
      if (
        exportOptions.includes("projects") &&
        reportData.projectsByTechnology
      ) {
        csv += "PROJECTS BY TECHNOLOGY\n";
        csv += "Technology,Count\n";
        Object.entries(reportData.projectsByTechnology).forEach(
          ([key, value]) => {
            csv += `${key},${value}\n`;
          }
        );
      ...
      if (
        exportOptions.includes("recentActivities") &&
      ...
      if (reportData.averageWorkingHours) {
        ...
        Object.entries(reportData.averageWorkingHours).forEach(
          ([intern, hours]) => {
            const internName = (intern || "N/A").replace(/,/g, ";");
      ...
      if (reportData.internsPerSupervisor) {
        ...
        Object.entries(reportData.internsPerSupervisor).forEach(
          ([supervisor, count]) => {
      ...
      if (reportData.instituteDistribution) {
        ...
        csv += "Institute,Count\n";
        Object.entries(reportData.instituteDistribution).forEach(
          ([institute, count]) => {
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute(
          "download",
          `SLT_Intern_Report_${new Date().toISOString().split("T")[0]}.csv`
        );
  ...
  const handleManualDownload = () => {
    if (manualDownloadUrl) {
      ...
      link.download = `SLT_Intern_Report_${
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600">
                            Automatic download failed. Please use the button
                            below.
                          </p>
                        </div>
                        <Button
                    ...
                    <Card>
                      <CardBody className="p-4">
                        ...
                        <CheckboxGroup
                          ...
                          className="space-y-2"
                        >
                          {Object.entries(exportOptionLabels).map(
                            ([key, label]) => {
                  ...
                  <CardBody className="p-4">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <div className="text-sm text-gray-600 space-y-1">
                        <p>
                          <strong>Format:</strong> {pdfSettings.format} -{" "}
                        ...
                        <p>
                          ...
                          {
                            colorSchemes.find(
                              (c) => c.key === pdfSettings.colorScheme
                            )?.label
                          }
                        </p>
                        <p>
                          <strong>Sections:</strong> {exportOptions.length}{" "}
                          selected
                        </p>
                        <p>
                          ...
                          {[
                            ...
                            pdfSettings.includeTableOfContents &&
                              "Table of Contents",
                          ]
                            .filter(Boolean)
                            .join(", ") || "None"}
