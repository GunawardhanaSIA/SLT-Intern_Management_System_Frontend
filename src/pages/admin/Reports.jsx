...
function Reports() {
  ...
  const pieChartData = reportData.internStatusDistribution
    ? Object.entries(reportData.internStatusDistribution).map(
        ([status, count]) => {
          const colors = {
            Active: "#10B981",
            Completed: "#3B82F6",
    ...
    : [
        {
          name: "Active",
          value: reportData.activeInterns || 0,
          color: "#10B981",
        },
        {
          name: "Completed",
          value: reportData.completedInterns || 0,
          color: "#3B82F6",
        },
        {
          name: "Dropped",
          value: reportData.droppedInterns || 0,
          color: "#EF4444",
        },
        {
          name: "Suspended",
          value: reportData.suspendedInterns || 0,
          color: "#F59E0B",
        },
      ];

  const monthlyData = Object.entries(reportData.monthlyRegistrations || {}).map(
    ([month, count]) => ({
  ...
  const supervisorStatusData = reportData.supervisorStatusDistribution
    ? Object.entries(reportData.supervisorStatusDistribution).map(
        ([status, count]) => {
          const colors = {
            Active: "#10B981",
            Inactive: "#EF4444",
    ...
    : [
        {
          name: "Active",
          value: reportData.activeSupervisors || 0,
          color: "#10B981",
        },
        {
          name: "Inactive",
          value: reportData.inactiveSupervisors || 0,
          color: "#EF4444",
        },
        {
          name: "On Leave",
          value: reportData.onLeaveSupervisors || 0,
          color: "#F59E0B",
        },
      ];

  const projectTechData = Object.entries(
    ...
    <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">
                  {reportData.totalInterns && reportData.totalInterns > 0
                    ? Math.round(
                        ((reportData.completedInterns || 0) /
                          reportData.totalInterns) *
                          100
                      )
                    : 0}
                  %
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card
          ...
          onClick={() =>
            openChartModal("pie", pieChartData, "Intern Status Distribution")
          }
        >
        ...
        <Card
          className="cursor-pointer hover:shadow-lg transition-shadow"
          onClick={() =>
            openChartModal("line", monthlyData, "Monthly Registration Trend")
          }
        >
          <CardHeader className="pb-2">
            <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">
                        {performer.performanceScore?.toFixed(1) || "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Total Hours:</span>
                      <span className="font-medium">
                        {performer.totalHours?.toFixed(1) || "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Attendance:</span>
                      <span className="font-medium">
                        {performer.attendanceRate?.toFixed(1) || "N/A"}%
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  .map((activity, index) => (
                    ...
                    >
                      ...
                      <Chip
                        size="sm"
                        color={
                          activity.type === "New Intern" ? "success" : "primary"
                        }
                        variant="flat"
                      >
                        {activity.type}
              ...
              <ModalBody>
                <div className="h-96 flex justify-center">
                  {selectedChart?.type === "pie" &&
                    selectedChart?.data?.some((item) => item.value > 0) && (
                      <MUIPieChart
                        series={[
                          {
