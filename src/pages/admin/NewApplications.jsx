...
const NewApplications = () => {
  ...
  const isAllSelected =
    visibleIds.every((id) => selectedIds.includes(id)) && visibleIds.length > 0;
  ...
  const handleRankResumes = () => {
    ...
    axios
      .post("http://localhost:8001/rank_cvs", payload, {
        headers: {
          "Content-Type": "application/json",
        },
      })
      .then((response) => {
        ...
        const formatted = response.data.rankedApplicants
          ...
          .map((app) => {
            const original = selectedApplicants.find(
  ...
  const handleSendToSystem = () => {
    const selected = rankedApplicants.filter((app) =>
      selectedRankedIds.includes(app.id)
    );
  ...
  const filteredApplicants = applicants.filter((applicant) => {
    ...
    const matchesSpecialization =
      ...
      applicant.specialization === selectedSpecialization;
  ...
  const handleSendEmail = async (id, email, date, time) => {
    ...
    try {
      const putResponse = await fetch(
        `http://localhost:8080/admin/setForInterview/${id}?interviewDate=${encodeURIComponent(
          date
        )}&interviewTime=${encodeURIComponent(time)}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="mx-6 mt-6">
      <div className="flex justify-between">
        ...
        <div className="flex gap-1 items-center">
          <Dropdown>
            <DropdownTrigger>
              <Button variant="flat" selectionMode="single">
                {selectedValue}
                <IoIosArrowDown />
              </Button>
            </DropdownTrigger>
      ...
      >
        <ModalContent>
          <ModalHeader className="flex flex-col gap-1 text-blue">
            {selectedApplicant
              ? `${`A${String(selectedApplicant.applicantId).padStart(
                  6,
                  "0"
          ...
          <ModalBody>
            {selectedApplicant ? (
              <div>
                ...
                <h1 className="pb-4 mt-16 font-semibold">
                ...
                <div className="grid grid-cols-2 gap-y-5 gap-x-10 m-auto">
                  <Input
                    ...
                    label="7. Institute"
                    labelPlacement="outside"
                    value={selectedApplicant.educationalInstitute}
                    type="text"
                  />
                  <Input
                    isDisabled
                    label="8. Degree/Course"
                    labelPlacement="outside"
                  ...
                  <Input
                    isDisabled
                    label="9. Current Academic Year"
                    labelPlacement="outside"
                    value={selectedApplicant.academicYear}
                    type="text"
                  />
                  <Input
                    isDisabled
                    label="10. Internship Period (Months)"
                    labelPlacement="outside"
                    value={selectedApplicant.internshipPeriod}
                    type="text"
                  />
                </div>
                <h1 className="pb-4 mt-16 font-semibold">
                ...
                <div className="grid grid-cols-2 gap-y-5 gap-x-10 m-auto">
                  <Input
                    isDisabled
                    label="11. Field of Specialization"
                    labelPlacement="outside"
                    value={selectedApplicant.specialization}
                    type="text"
                  />
                  <Input
                    isDisabled
                    label="12. Programming Languages"
                    labelPlacement="outside"
                    value={selectedApplicant.programmingLanguages}
                    type="text"
                  />
                </div>
        ...
        <ModalContent>
          ...
          <ModalBody className="flex flex-col gap-8">
            <Input
              ...
              labelPlacement="outside"
              placeholder={interviewee ? interviewee.name : ""}
              type="text"
            />
            <DateInput
              className="max-w-sm"
          ...
          <ModalFooter>
            <Button
              color="danger"
              variant="light"
              onPress={handleCloseInterviewModal}
            >
              Close
            </Button>
            <Button
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button
              color="danger"
              variant="light"
              onPress={() => setIsRankModalOpen(false)}
            >
              Close
            </Button>
            <Button
              ...
              isDisabled={selectedRankedIds.length === 0}
            >
              Send Selected
            </Button>
          </ModalFooter>
