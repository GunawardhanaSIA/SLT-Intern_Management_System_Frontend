...
const ManageInterns = () => {
    ...
    if (sortedInterns.length > 0) {
      const filtered = sortedInterns.filter((intern) => {
        ...
        return (
          ...
          `I${String(intern.internId).padStart(6, "0")}`
            .toLowerCase()
  ...
  const fetchInterns = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/admin/interns`, {
        headers: {
  ...
  const handleSave = async () => {
    try {
      const token = getToken();
      await axios.put(
        `${API_BASE_URL}/admin/interns/${editingIntern.internId}`,
        editingIntern,
        {
          headers: {
            Authorization: `Bearer ${token}`,
                ...
                <td>
                  {editingIntern?.internId === intern.internId ? (
                    <Select
                      size="sm"
                      selectedKeys={
                        editingIntern.supervisorId
                          ? [editingIntern.supervisorId.toString()]
                          : []
                      }
                    ...
                    <div className="relative flex items-center gap-2">
                      ...
                      <Tooltip content="Deactivate">
                        <span className="text-lg text-[#EF4444] cursor-pointer active:opacity-50">
                          <FaTrash
                            onClick={() => handleDelete(intern.internId)}
                          />
                        </span>
                      </Tooltip>
                    </div>
      ...
      >
        <ModalContent>
          <ModalHeader className="flex flex-col gap-1 text-blue">
            {selectedIntern
              ? `${`I${String(selectedIntern.internId).padStart(6, "0")}`} - ${
                  selectedIntern.name
                }`
              : "Intern Details"}
          </ModalHeader>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-y-5 gap-x-7 m-auto">
                  ...
                  <Input
                    ...
                    label="Mobile Number"
                    labelPlacement="outside"
                    value={selectedIntern.mobileNumber}
                    type="text"
                  />
                  <Input
                    isDisabled
                  ...
                  <Input
                    ...
                    label="Address"
                    labelPlacement="outside"
                    value={selectedIntern.address}
                    type="text"
                    className="col-span-2"
                  />
                </div>
                <h1 className="pb-4 mt-8 font-semibold">
                ...
                <div className="grid grid-cols-2 gap-y-5 gap-x-7 m-auto">
                  <Input
                    ...
                    label="Institute"
                    labelPlacement="outside"
                    value={selectedIntern.educationalInstitute}
                    type="text"
                  />
                  <Input
                    isDisabled
                  ...
                  <Input
                    ...
                    labelPlacement="outside"
                    value={selectedIntern.academicYear}
                    type="text"
                  />
                  <Input
                    isDisabled
                    label="Internship Period (Months)"
                    labelPlacement="outside"
                    value={selectedIntern.internshipPeriod}
                    type="text"
                  />
                </div>
                <h1 className="pb-4 mt-8 font-semibold">
                ...
                <div className="grid grid-cols-2 gap-y-5 gap-x-7 m-auto">
                  ...
                  <Input
                    ...
                    labelPlacement="outside"
                    value={selectedIntern.programmingLanguages}
                    type="text"
                  />
                  <Input
                    isDisabled
                    label="Start Date"
                    labelPlacement="outside"
                    value={selectedIntern.startDate}
                    type="text"
                  />
                  <Input
                    isDisabled
                    label="Supervisor"
                    labelPlacement="outside"
                    value={selectedIntern.supervisor?.name || "Not Assigned"}
                    type="text"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 gap-y-5 gap-x-7 m-auto">
                  <Input
                    ...
                    labelPlacement="outside"
                    value={getStatusBadge(selectedIntern.state).props.children}
                    type="text"
                  />
                </div>
              </div>
