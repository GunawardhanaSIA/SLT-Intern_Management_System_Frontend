...
const ForgotPassword = () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (emailResponse.status === 200) {
          ...
          const otpStoreResponse = await axios.post(
            `${API_BASE_URL}/storeOTP`,
            {
              email: email,
              otp: otp,
            }
  ...
  return (
    <div>
      ...
      <p className="text-gray flex text-center mt-1">
      ...
      {errorMessage && (
        <p className="text-red font-light text-xs mt-4">{errorMessage}</p>
      )}

      <div className="mt-8">
        <Button
          className="w-full bg-blue font-bold text-white"
