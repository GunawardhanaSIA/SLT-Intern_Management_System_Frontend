...
const Signin = () => {
  ...
  const handleGoogleSignUpSuccess = async (token) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/signup/google`, {
        token: token,
      });
  ...
  return (
    <div>
      <h1 className="text-2xl flex font-semibold justify-center">
        Welcome Back!
      </h1>
      ...
      <p className="text-xs font-light cursor-pointer flex justify-end mb-2">
        Forgot Password?
      </p>

      {errorMessage && (
        <p className="text-red font-light text-xs my-4">{errorMessage}</p>
      )}
      ...
      <p className="flex gap-2 justify-center mt-2 text-sm text-gray">
