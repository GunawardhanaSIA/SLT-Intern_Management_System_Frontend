...
const Attendance = () => {
  ...
  const handleMarkAttendance = (internId) => {
    ...
    axios
      .post(
        `${API_BASE_URL}/supervisor/attendance?internId=${internId}&date=${formattedDate}`,
        {},
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const handlePastAttendance = (internId, internName) => {
    ...
    axios
      .get(`${API_BASE_URL}/supervisor/attendance/${internId}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
  ...
  return (
    <div className="mx-6 mt-6">
      ...
      {/* Attendance Table */}
      <div
        className="attendance_table mt-10 flex justify-center"
        role="region"
        tabIndex="0"
      >
          ...
          <tbody className="text-xs align-center">
            {filteredInterns.map((intern) => (
              <tr key={intern.internId}>
                ...
                <td>
                  {calculateEndDate(intern.startDate, intern.internshipPeriod)}
                </td>
                <td>
                  <Button
                    size="sm"
                    className={`text-white font-bold ${
                      markedAttendance[intern.internId]
                        ? "bg-yellow-500"
                        : "bg-green"
                    }`}
                  ...
                  <Button
                    size="sm"
                    className="ml-2 text-blue bg-white font-bold"
                    onClick={() =>
                      handlePastAttendance(intern.internId, intern.name)
                    }
                  >
                    Past Attendance
                  </Button>
      ...
      >
        <ModalContent>
          <ModalHeader className="text-blue">
            {selectedInternName} - Past Attendance
          </ModalHeader>
